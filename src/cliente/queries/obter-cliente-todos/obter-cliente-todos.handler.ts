...
export class ObterClienteTodosHandler implements
    IQueryHandler<ObterClienteTodosCommand, Result<ObterClienteTodosResponse[]>> {
    ...
    async execute(query: ObterClienteTodosCommand): Promise<Result<ObterClienteTodosResponse[]>> {
        ...
        const clientes: Cliente[] = await this.em.find(
            Cliente,
            {
                Ativo: query.ativos
            });
